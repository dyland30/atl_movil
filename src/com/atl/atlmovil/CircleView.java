...
implements OnClickListener
{
    ...
    public void onDraw(Canvas canvas) {
    	...
    	pRect.setColor(getResources().getColor(R.color.white));
    	
    	pRect.setStyle(Paint.Style.FILL);
    	pRect.setAntiAlias(true);
    	
    	
    	canvas.drawRect(0,0,w,h,pRect);
    	
    	canvas.drawCircle(ox, oy, rad, getBrush());
    	canvas.drawText("Prueba ", 10, 20, getTextBrush());
    ...
    private Paint getBrush()
    {
    	...
    	p.setColor(getResources().getColor(R.color.bluegrass));
    	p.setStyle(Paint.Style.STROKE);
    	return p;
    }
    
    private Paint getTextBrush()
    {
    	Paint p = new Paint();
    	p.setAntiAlias(true);
    	p.setStrokeWidth(strokeWidth);
    	p.setColor(getResources().getColor(R.color.seafoam));
